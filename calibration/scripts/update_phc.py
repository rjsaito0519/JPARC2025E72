...
def make_dictdata(root_file_path, good_ch_range = [-np.inf, np.inf]):
    ...
    elif "T1" in root_file_path:
        detector_id = detector_id_list["T1"]
    ...
    for i in range(len(tree["p0_val"])):
        ...
        if good_ch_range[0] <= ch and ch <= good_ch_range[1]:
            # Determine number of entries (1 for single side, 2 for both)
            n_entries = len(tree["p0_val"][i])
            for UorD in range(n_entries):
                ...
                for j in range(3):
                    if len(buf) > UorD: # Ensure buf has enough lists
    ...
    for i in bad_ch_index:
        ch = tree["ch"][i]
        # Determine max UorD from available data if possible, or just try 0 and 1
        # Use buf content check
        for UorD in [0, 1]:
            if len(buf) <= UorD or not buf[UorD][0]: continue 
