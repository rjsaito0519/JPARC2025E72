...
# Current file: <RepoRoot>/calibration/scripts/run_hdprm.py
...
def main():
    parser = argparse.ArgumentParser(
        prog="run_hdprm",
        description="Run multiple HDPRM analysis binaries for a given ROOT file or run number.",
        add_help=True,
    )
    ...
    parser.add_argument("--ftof", action="store_true", help="Include Forward TOF (CVC, SAC3, SFV) analysis")
    ...
    # 2. Define standard HDPRM binaries
    # Directory structure: <RepoRoot>/bin/
    bin_dir = project_root / "bin"
    
    standard_detectors = ["BHT", "BH2", "BAC", "KVC", "T1"]
    ftof_detectors = ["CVC", "SAC3", "SFV"]
    
    detectors_to_run = standard_detectors
    if args.ftof:
        detectors_to_run += ftof_detectors
        
    print(colored(f"\n>>> Starting HDPRM Analysis for Run {run_num} ({args.suffix}) <<<", "green", attrs=["bold"]))
    print(colored(f"Input file: {hodo_decoded_file}\n", "white"))

    failed = []
    for det in detectors_to_run:
        binary_name = f"{det}_HDPRM"
    ...
    print("\n" + "="*50)
    if not failed:
        print(colored("All requested HDPRM analyses completed successfully!", "green", attrs=["bold"]))
    else:
    ...
    if args.update and not failed:
        ...
        run_command([sys.executable, str(update_script), str(run_num), args.suffix, "hdprm"])
