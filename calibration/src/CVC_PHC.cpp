...
void analyze(TString path, TString particle){    
    ...
    TString output_path = Form("%s/run%05d_CVC_PHC_%s.root", root_base_dir.Data(), run_num, particle.Data());
    ...
    TH2D *h_cvc_cftof_vs_de[2][conf.num_of_ch.at("cvc")];
    ...
    TString pdf_path = Form("%s/run%05d_CVC_PHC_%s.pdf", img_base_dir.Data(), run_num, particle.Data());
