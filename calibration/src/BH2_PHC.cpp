...
void analyze(TString path, TString particle){    
    ...
    TString output_path = Form("%s/run%05d_BH2_PHC_%s.root", root_base_dir.Data(), run_num, particle.Data());
    ...
    TH2D *h_bh2_cbtof_vs_de[2][conf.num_of_ch.at("bh2")];
    for (Int_t i = 0; i < conf.num_of_ch.at("bh2"); i++ ) {
        h_bh2_btof_vs_de[0][i]  = (TH2D*)f->Get(Form("T0_seg%dU_BTOF_vs_DeltaE_%s", i, particle.Data()));
        h_bh2_btof_vs_de[1][i]  = (TH2D*)f->Get(Form("T0_seg%dD_BTOF_vs_DeltaE_%s", i, particle.Data()));
        h_bh2_cbtof_vs_de[0][i] = (TH2D*)f->Get(Form("T0_seg%dU_CBTOF_vs_DeltaE_%s", i, particle.Data()));
        h_bh2_cbtof_vs_de[1][i] = (TH2D*)f->Get(Form("T0_seg%dD_CBTOF_vs_DeltaE_%s", i, particle.Data()));
    }
    ...
    TString pdf_path = Form("%s/run%05d_BH2_PHC_%s.pdf", img_base_dir.Data(), run_num, particle.Data());
