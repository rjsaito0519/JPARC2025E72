...
void analyze(TString path, TString particle){    
    ...
    TString output_path = Form("%s/run%05d_BLC%s_residual_%s.root", root_base_dir.Data(), run_num, in_or_out.Data(), particle.Data());
    ...
    TString pdf_path = Form("%s/run%05d_BLC%s_residual_%s.pdf", img_base_dir.Data(), run_num, in_or_out.Data(), particle.Data());
