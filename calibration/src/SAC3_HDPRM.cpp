...
void analyze(TString path, TString particle){    
    ...
    TString output_path = Form("%s/run%05d_SAC3_HDPRM_%s.root", root_base_dir.Data(), run_num, particle.Data());
    ...
    // +-------------------+    
    // -- tdc ----------
    TH1D *h_sac3_tdc[1][conf.num_of_ch.at("sac3")];
    for (Int_t i = 0; i < conf.num_of_ch.at("sac3"); i++ ) h_sac3_tdc[0][i] = (TH1D*)f->Get(Form("SAC3_TDC_seg%dU_%s", i, particle.Data()));
    // -- adc ----------
    TH1D *h_sac3_adc[1][conf.num_of_ch.at("sac3")];
    for (Int_t i = 0; i < conf.num_of_ch.at("sac3"); i++ ) h_sac3_adc[0][i] = (TH1D*)f->Get(Form("SAC3_ADC_seg%dU_%s", i, particle.Data()));

    // -- set tdc range ----------
    TH1D *h_sum_tdc = (TH1D*)h_sac3_tdc[0][0]->Clone("h_sum_tdc");
    h_sum_tdc->Reset(); 
    for (Int_t i = 0; i < conf.num_of_ch.at("sac3"); i++) {
        h_sum_tdc->Add(h_sac3_tdc[0][i]);
    }
    ...
    TString pdf_path = Form("%s/run%05d_SAC3_HDPRM_%s.pdf", img_base_dir.Data(), run_num, particle.Data());

    // -- container -----
    std::vector<FitResult> adc_up;
    std::vector<FitResult> tdc_up;

    auto c_sac3 = new TCanvas("sac3", "", 1500, 1200);
    c_sac3->Divide(cols, rows);
    c_sac3->Print(pdf_path + "["); // start
    nth_pad = 1;
    for (Int_t i = 0; i < conf.num_of_ch.at("sac3"); i++) {
        if (nth_pad > max_pads) {
            c_sac3->Print(pdf_path);
            c_sac3->Clear();
            c_sac3->Divide(cols, rows);
            nth_pad = 1;
        }
