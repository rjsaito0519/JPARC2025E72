...
void analyze(TString path, TString particle){    
    ...
    TString output_path = Form("%s/run%05d_HTOF_PHC_%s.root", root_base_dir.Data(), run_num, particle.Data());
    ...
    TString pdf_path = Form("%s/run%05d_HTOF_PHC_%s.pdf", img_base_dir.Data(), run_num, particle.Data());
