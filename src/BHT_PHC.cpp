...
void analyze(TString path, TString particle){    
    ...
    // +--------------------------+
    TString output_path = Form("%s/root/run%05d_BHT_PHC_%s.root", OUTPUT_DIR.Data(), run_num, particle.Data());
    ...
    for (Int_t i = 0; i < conf.num_of_ch.at("bht"); i++ ) h_bht_btof_vs_de[0][i] = (TH2D*)f->Get(Form("BHT_seg%dU_CBTOF_vs_DeltaE_%s", i, particle.Data()));
    for (Int_t i = 0; i < conf.num_of_ch.at("bht"); i++ ) h_bht_btof_vs_de[1][i] = (TH2D*)f->Get(Form("BHT_seg%dD_CBTOF_vs_DeltaE_%s", i, particle.Data()));
    ...
    TString pdf_path = Form("%s/img/run%05d_BHT_PHC_%s.pdf", OUTPUT_DIR.Data(), run_num, particle.Data());
