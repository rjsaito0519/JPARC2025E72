...
void analyze(TString path, TString particle){    
    ...
    for (Int_t i = 0; i < conf.num_of_ch.at("bh2"); i++ ) h_bh2_btof_vs_de[0][i] = (TH2D*)f->Get(Form("T0_seg%dU_BTOF_vs_DeltaE_%s", i, particle.Data()));
    for (Int_t i = 0; i < conf.num_of_ch.at("bh2"); i++ ) h_bh2_btof_vs_de[1][i] = (TH2D*)f->Get(Form("T0_seg%dD_BTOF_vs_DeltaE_%s", i, particle.Data()));
