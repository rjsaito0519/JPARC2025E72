...
void analyze(TString path, TString particle){    
    ...
    // +--------------------------+
    TString output_path = Form("%s/root/run%05d_T1_HDPRM_%s.root", OUTPUT_DIR.Data(), run_num, particle.Data());
    ...
    // +-------------------+    
    // -- tdc ----------
    TH1D *h_t1_tdc[1][conf.num_of_ch.at("t1")];
    for (Int_t i = 0; i < conf.num_of_ch.at("t1"); i++ ) h_t1_tdc[0][i] = (TH1D*)f->Get(Form("T1_TDC_seg%dU_%s", i, particle.Data()));
    // -- adc ----------
    TH1D *h_t1_adc[1][conf.num_of_ch.at("t1")];
    for (Int_t i = 0; i < conf.num_of_ch.at("t1"); i++ ) h_t1_adc[0][i] = (TH1D*)f->Get(Form("T1_ADC_seg%dU_%s", i, particle.Data()));

    // -- set tdc range ----------
    TH1D *h_sum_tdc = (TH1D*)h_t1_tdc[0][0]->Clone("h_sum_tdc");
    h_sum_tdc->Reset(); 
    for (Int_t i = 0; i < conf.num_of_ch.at("t1"); i++) {
        h_sum_tdc->Add(h_t1_tdc[0][i]);
    }
    ...
    TString pdf_path = Form("%s/img/run%05d_T1_HDPRM_%s.pdf", OUTPUT_DIR.Data(), run_num, particle.Data());
