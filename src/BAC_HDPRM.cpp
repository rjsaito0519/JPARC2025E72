...
void analyze(TString path, TString particle){    
    ...
    // +-------------------+    
    // -- adc ----------
    TH1D *h_adc[4]; // BAC seg0-3 U
    for (Int_t i = 0; i < 4; i++ ) {
        h_adc[i] = (TH1D*)f->Get(Form("%s_ADC_seg%dU_%s", counter_upper.Data(), i, particle.Data()));
    }
    // -- tdc ----------
    // BAC has only seg4U TDC (Sum)
    TH1D *h_tdc_sum = (TH1D*)f->Get(Form("%s_TDC_seg4U_%s", counter_upper.Data(), particle.Data()));
